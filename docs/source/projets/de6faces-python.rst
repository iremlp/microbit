...
Progression
===========
Pour afficher aléatoirement un nombre entier entre 1 et 6, on peut se contenter
des 3 lignes ci-dessous.
::

  while True:
    if button_a.get_presses():
        display.show(str(random.randint(1, 6)))  # "str" car "display.show" n'affiche que du texte

Ce qui n'a pas d'autre intérêt que d'introduire la fonction "randint".
Contrairement à la progression par utilisée dans le projet avec les blocs, on suggère
ici de commencer par l'affichage tel qu'il est sur un vrai dé.
Cela qui permet de réexploiter les listes d'images introduites avec le projet pile ou face.
